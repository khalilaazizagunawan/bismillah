...
(async () => {
  try {
    ...
    console.error("❌ Procurement DB connection failed:", err.message);
    setTimeout(async () => {
      try {
        await pool.query("SELECT 1");
        console.log("📦 Procurement DB connected (retry)");
      } catch (retryErr) {
        console.error("❌ Procurement DB connection failed on retry:", retryErr.message);
        process.exit(1);
      }
    }, 5000);
  }
...
process.on("SIGTERM", async () => {
  console.log("🔻 Closing Procurement DB pool...");
  await pool.end();
  process.exit(0);
});
